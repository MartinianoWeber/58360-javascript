...
const borrarTodoBtn = document.querySelector('#borrarTodo')
    ...
    datosArr.forEach((dato) => {
        ...
        <div class="simulador__formularioFin-actualizacion">
            <div class="simulador__formularioFin-actualizacion-datos">
                ...
                <button class="botonBorrar" id="${dato.id}">X</button>
            </div>
...
function borrarTarjeta(selectores) {
    selectores.forEach((selector) => {
        selector.addEventListener('click', (e) => {
            const id = parseInt(e.target.id)
            console.log(id)
            const elemento = datosArr.map((dato) => dato.id).indexOf(id)
            datosArr.splice(elemento, 1)
            localStorage.setItem("datos", JSON.stringify(datosArr))
            crearTemplate()
