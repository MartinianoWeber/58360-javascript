...
function buscarUnPerro() {
  ...
  if (perroEncontrado === false) {
    ...
    alert(
      "Si se puede encontrar el perro en nuestra base pero por politicas de seguridad no podemos mostrarlo"
    );
    initProgram();
  }
}
...
function verNombresPerrosAlReves() {
  const nombresPerros = perritosArr
    .map((perro) => perro.name)
    .reverse()
    .join(" - ");
  alert(nombresPerros);
...
function adoptarPerroEspecificoSlice() {
  let nombre = prompt(
    "Introduce el nombre del perspectiveOrigin:  que quieres buscar"
  );
  const perroEncontrado = perritosArr
    .map((perro) => perro.name)
    .indexOf(nombre);
  if (perroEncontrado == -1) {
    ...
    alert(
      `El perro ${perritosArr[perroEncontrado].name} esta en la posicion ${perroEncontrado}`
    );
